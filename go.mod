...
func createContainer(w http.ResponseWriter, r *http.Request) {
	...
	net0 := request.Net0
	if net0 == "" {
		generatedIP, generatedIPv6 := generateIP(vmid)
		ip = generatedIP
		ipv6 = generatedIPv6
		net0 = fmt.Sprintf("name=eth0,bridge=vmbr1,ip=%s/24,gw=%s,ip6=%s/64,gw6=%s", ip, config.Gateway, ipv6, config.IPv6Gateway)
		log.Printf("未提供 net0，已生成: %s", net0)
	} else {
		parts := strings.Split(net0, ",")
		for _, part := range parts {
			if strings.HasPrefix(part, "ip=") && !strings.HasPrefix(part, "ip6=") {
				ip = strings.Split(strings.TrimPrefix(part, "ip="), "/")[0]
			}
			if strings.HasPrefix(part, "ip6=") {
				ipv6 = strings.Split(strings.TrimPrefix(part, "ip6="), "/")[0]
			}
		}
	}
	...
	result, err := apiRequest("POST", "/api2/json/nodes/"+node+"/lxc", data)
	if err != nil {
	...
	_, err = db.Exec(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, vmid, request.Hostname, password, ip, ipv6, request.Memory, request.CPU, request.Disk, "stopped")
	if err != nil {
